...
def _pomo_chime(kind: str = "bell", volume: float = 0.6) -> None:
    """Сигнал окончания помодоро на колонках ПК — слышно, даже если озвучка выключена или занята.
    bell — три мягких удара, ding — один короткий, tick — быстрые щелчки; off — тишина."""
    if kind == "off":
        return
    vol = max(0.0, min(1.0, float(volume or 0.6))) * 0.5
    try:
        import sounddevice as sd
        sr = 48000
        def tone(freq, ms, decay=6.0):
            t = np.linspace(0, ms / 1000, int(sr * ms / 1000), False)
            return (np.sin(2 * np.pi * freq * t) * np.exp(-decay * t / (ms / 1000)) * vol).astype(np.float32)
        gap = np.zeros(int(sr * 0.12), dtype=np.float32)
        if kind == "ding":
            wave = tone(1320, 350)
        elif kind == "tick":
            wave = np.concatenate([np.concatenate([tone(2000, 40, 12), np.zeros(int(sr * 0.09), dtype=np.float32)]) for _ in range(6)])
        else:
            wave = np.concatenate([tone(880, 500), gap, tone(1100, 500), gap, tone(1320, 800)])
                ...
                with c.stream("GET", f"{API}/api/events/stream", params={"client": "pc"}) as r:
                    ...
                    for line in r.iter_lines():
                        ...
                        if ev.get("kind") == "pc":
                            ...
                            continue
                        if ev.get("kind") == "timer" and ev.get("done"):
                            # помодоро/перерыв истёк: сигнал на колонки сразу, голосом скажет reminder-событие (если включено)
                            threading.Thread(target=_pomo_chime, args=(ev.get("sound", "bell"), ev.get("volume", 0.6)), daemon=True, name="pomo-chime").start()
                            continue
                        ...
                        if len(seen) > 500:
                            seen.clear()
                        if _is_night() and not any(m in ev["text"] for m in ("🚨", "⏰", "⏳", "🍅")):
