...
const COLORS = {
  ...
  debt: 'var(--neg)', goal: 'var(--pos)', task: 'var(--ink-2)', event: 'var(--ink-2)',
}
const KIND_RU = { person: 'люди', order: 'заказы', note: 'мысли', link: 'ссылки', tag: 'теги', debt: 'долги', goal: 'цели', task: 'задачи', event: 'встречи' }
const LEGEND = ['person', 'order', 'note', 'link', 'tag', 'debt']
...
export default function Graph({ height = 520, focus: initialFocus = null, compact = false }) {
    ...
    const step = () => {
      ...
      if (alpha > 0.005) {
        ...
        for (const e of edges) {
          ...
          const want = e.rel === 'tag' ? 70 : e.rel === 'similar' ? 90 : 60
      ...
      for (const e of edges) {
        ...
        ctx.lineWidth = e.rel === 'client' || e.rel === 'wiki' ? 1.6 : 1
      ...
      for (const n of nodes) {
        ...
        if (n.kind === 'note' || n.kind === 'tag') { ctx.fillStyle = bg; ctx.beginPath(); ctx.arc(n.x, n.y, Math.max(1, n.r - 2), 0, Math.PI * 2); ctx.fill(); ctx.fillStyle = palette[n.kind]; ctx.beginPath(); ctx.arc(n.x, n.y, Math.max(0.5, n.r - 3.5), 0, Math.PI * 2); ctx.fill() }
  ...
  const onMove = (e) => {
    ...
    if (d?.node) { const p = toWorld(e); d.node.x = p.x; d.node.y = p.y; d.moved = true; return }
    ...
    const n = pick(toWorld(e)); if (n?.id !== hover?.id) setHover(n || null)
  }
  const onUp = (e) => {
  ...
  const onDbl = (e) => {
    ...
    const to = { person: `/people?id=${n.ref_id}`, order: '/orders', note: `/mind?q=${encodeURIComponent(n.label.slice(0, 40))}`, link: n.url || '/mind', tag: `/mind?q=${encodeURIComponent(n.label)}`, debt: '/finance', goal: '/finance', task: '/tasks', event: '/calendar' }[n.kind]
    if (!to) return
  ...
  return (
    <div className="relative">
        ...
        <div className="pointer-events-none absolute left-3 top-3 flex flex-wrap gap-1.5">
          {LEGEND.map((k) => (
            <button key={k} type="button" className={`pointer-events-auto chip !bg-[var(--bg)] ${hidden.has(k) ? 'opacity-40' : ''}`} onClick={() => setHidden((h) => { const n = new Set(h); n.has(k) ? n.delete(k) : n.add(k); return n })}>
              <span className="h-2 w-2 rounded-full" style={{ background: COLORS[k] }} />{KIND_RU[k]}
            </button>
          ))}
        </div>
      )}
      ...
      {hover && <div className="pointer-events-none absolute bottom-3 left-3 max-w-[70%] rounded-xl px-3 py-1.5 text-[12.5px]" style={{ background: 'var(--ink)', color: 'var(--bg)' }}>{KIND_RU[hover.kind]?.replace(/и$|ы$/, '') || hover.kind} · {hover.label}{hover.deg ? ` · связей: ${hover.deg}` : ''}</div>}
