...
def _clean(text: str) -> str:
    ...
    t = re.sub(r"\s{2,}", " ", t)
    t = re.sub(r"\s+([,.;:!?])", r"\1", t)   # «с Димой , кофейня» → «с Димой, кофейня» (пробел остался от вырезанной даты)
    return t.strip(" ,.-—:")
...
def parse_datetime_ex(text: str, now: datetime | None = None) -> tuple[datetime | None, str, bool]:
    ...
    if m:
        ...
        # «10к» (тысячи) — не время; «в 10 к врачу» — время (пробел перед «к»)
        m = re.search(r"(?:в|на|к)\s+(\d{1,2})(?:\s*(?:час\w*|ч\b))?(?![\dк])(?!\s*(?:сентябр|октябр|ноябр|декабр|январ|феврал|март|апрел|ма[йя]|июн|июл|август|числ|минут|тыс|руб|₽|%|-го|го\b))\s*(утра|дня|вечера|ночи)?", t) \
