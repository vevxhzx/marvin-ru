...
def test_pomodoro_settings_auto_break_and_api():
    ...
    with db.session() as s:
        ...
        row = s.get(db.WorkSession, w.id); row.started_at = datetime.now() - timedelta(minutes=26); s.add(row); s.commit()
    ev = orders.due_timer_ping()
    ...
    with db.session() as s:
        w = orders.start_session(None, 25)
        row = s.get(db.WorkSession, w.id); row.started_at = datetime.now() - timedelta(minutes=1); s.add(row); s.commit()
    orders.stop_session()
    n = orders.today_sessions()
    with db.session() as s:
        w = orders.start_session(None, 25)
        row = s.get(db.WorkSession, w.id); row.started_at = datetime.now() - timedelta(minutes=24); s.add(row); s.commit()
    orders.stop_session()
    assert orders.today_sessions() == n + 1
    # каждый 2-й помидор — длинный перерыв
